// Mock Func
import type { ResourceEntity, ResourceWithAccessKey } from '@/api/course/CourseResourceAPI'
...
// Mock 获取资源元数据
export function mockGetResourceMetaDataCall(id: number, data: APIResult<ResourceWithAccessKey>) {
...
// Mock 获取章节下的所有资源
export function mockGetResourcesByChapterCall(chapterId: number, data: APIResult<{ content: ResourceWithAccessKey[] }>) {
  const url = `${service_backend_base}/chapter/${chapterId}/resource`;
  setMockFunc(url, RequestType.GET, null, () => data);
}
